...
import { getStatsByCategory, getExpenseStatistics, ChartDataPoint, CATEGORY_COLORS } from '../utils/chartHelper';
...
export default function BudgetChart({
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.card }]}>
            <Text style={[styles.title, { color: colors.text }]}>費用統計</Text>
            ...
            <View style={styles.barContainer}>
                {data.map((item, index) => {
                    ...
                    // 決定是否顯示堆疊圖
                    const isStacked = selectedCategory === 'all' && item.breakdown && item.breakdown.length > 0;

                    // 若非堆疊圖，決定單一顏色
                    const barColor = selectedCategory !== 'all'
                                ...
                                {isStacked ? (
                                    ...
                                    <View style={[styles.stackedBarContainer, { height: barHeight }]}>
                                        {item.breakdown!.map((segment, i) => {
                                            const segmentHeight = item.value > 0
                                                ? (segment.value / item.value) * 100
                                                : 0;
