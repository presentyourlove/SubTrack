...
describe('GenericChart', () => {
  ...
  describe('Bar chart rendering', () => {
    ...
    it('renders bar chart with title', () => {
      const { getByText } = render(<GenericChart data={barData} type="bar" title="Bar Chart" />);

      expect(getByText('Bar Chart')).toBeTruthy();
    });

    it('renders bar chart with custom height', () => {
      const { toJSON } = render(<GenericChart data={barData} type="bar" height={300} />);
  ...
  describe('Props handling', () => {
    it('uses default height when not specified', () => {
      const { toJSON } = render(<GenericChart data={[{ label: 'Test', value: 100 }]} type="bar" />);
    ...
    it('renders without title when not provided', () => {
      const { queryByText } = render(
        <GenericChart data={[{ label: 'Test', value: 100 }]} type="pie" />,
      );
