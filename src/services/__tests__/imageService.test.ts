...
describe('imageService', () => {
  ...
  describe('compressAndConvertImage', () => {
    ...
    it('throws error on manipulation failure', async () => {
      ...
      mockImageManipulator.manipulateAsync.mockRejectedValue(new Error('Manipulation failed'));
  ...
  describe('deleteCustomIcon', () => {
    ...
    it('logs error on delete failure', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('Failed to delete custom icon:', expect.any(Error));
      consoleSpy.mockRestore();
    });
  });
});
